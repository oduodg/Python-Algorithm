...
n번째 글자를 기준으로 오름차순 정렬 → key 함수의 lambda를 이용하자.

- 주의) n번째 글자가 같은 문자열일 경우 → 사전순으로 정렬해야하므로, 먼저 sorted()로 사전순 정렬을 시킨다.
- 사전순으로 정렬시킨 리스트를 key함수의 lambda를 사용해서 n번째 글자를 기준으로 다시 정렬시켜 리턴한다.

→ n번째 글자를 기준으로 오름차순 정렬 후, 사전 순으로 정렬하자라고 생각했지만 반대로 생각해서 사전 순으로 정렬하고 n번째 글자를 기준으로 오름차순 정렬하는 순서로 코드를 짜야한다.